...
impl Args {
    ...
    pub fn security_bytes(&self) -> Option<u32> {
        if self.no_security {
            None
        } else {
...
impl From<Args> for ProtocolWrapper {
...
pub mod tests {
    ...
    fn test_security_default() {
        let args = Args::try_parse_from(&["binary"]).unwrap();
        assert_eq!(args.security_bytes(), Some(40));
    }

    #[test]
    fn test_security_no_security() {
        let args = Args::try_parse_from(&["binary", "--no-security"]).unwrap();
        assert_eq!(args.security_bytes(), None);
    }

    #[test]
    fn test_security_custom_security() {
        let args = Args::try_parse_from(&["binary", "--security", "30"]).unwrap();
    ...
    fn test_security_conflicts() {
        assert!(
            Args::try_parse_from(&["binary", "--security", "30", "--no-security"]).is_err(),
